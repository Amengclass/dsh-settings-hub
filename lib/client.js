...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function HubSectionOutlet({ ctx, ownerProps, only }) {
			...
			// Check if this section still exists in our discovered map.
			const sectionStillExists = Object.values(pluginSectionMap).some((ids) => ids.includes(only));
			...
			const rendered = react.useMemo(() => {
				...
				if (winners.length === 0) {
					throw new Error(`Section '${only}' not found. The plugin may have been uninstalled.`);
				}
				const hit = winners[0];
