...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * Scope-based grouping: sub-dependencies sharing a scope with an
		 * installed plugin are merged under that plugin's group.
		...
		/**
		 * Build a map: module ID → parent plugin package name.
		 * Scoped modules match by scope prefix (e.g. @linxin666/dsh-pet
		 * → @linxin666/dsh-web-ui-all). Unscoped modules match by name.
		 */
		function buildParentMap() {
			const scopes = {}; // scope prefix → plugin package name
			const names = {};  // exact name → plugin package name
			for (const pkg of INSTALLED_PLUGINS) {
				const slash = pkg.indexOf('/');
				if (pkg.startsWith('@') && slash !== -1) {
					scopes[pkg.substring(0, slash + 1)] = pkg;
				} else {
					names[pkg] = pkg;
				}
			}
			return function getParent(modId) {
				const scopeKey = modId.startsWith('@') ? modId.substring(0, modId.indexOf('/') + 1) : null;
				if (scopeKey && scopes[scopeKey]) return scopes[scopeKey];
				if (names[modId]) return names[modId];
				return null; // not a direct or sub dependency of any installed plugin
			};
		}
