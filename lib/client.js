...
// __DEP_MAP_END__
		...
		function HubPanel({ rows, ctx, activeId, onSelect, onClose }) {
			...
			// Intercept DSH's native error messages (404 etc.) → replace with friendly text.
			react.useEffect(() => {
				const replaceErrors = () => {
					document.querySelectorAll('*').forEach((el) => {
						if (el._dshubFixed || !el.children.length) return;
						const t = el.textContent || '';
						if (t.includes('加载失败') && t.includes('HTTP')) {
							el._dshubFixed = true;
							el.style.cssText = 'padding:24px;text-align:center;';
							el.innerHTML = '<div style="font-size:14px;color:var(--dsw-alias-label-tertiary);margin-bottom:8px;">加载失败</div><div style="font-size:12px;color:var(--dsw-alias-label-tertiary);">该插件设置项可能已被卸载，请重启 DeepSeek Harness</div>';
						}
					});
				};
