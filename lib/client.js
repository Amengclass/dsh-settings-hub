...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Fetch installed plugin names from the plugin-manager gateway API,
		 * then scan each plugin's loadCache module for settings.section IDs.
		 */
		const installedPluginNamesCache = { promise: null, names: null };
		function getInstalledPluginNames() {
			if (installedPluginNamesCache.names) return Promise.resolve(installedPluginNamesCache.names);
			if (!installedPluginNamesCache.promise) {
				installedPluginNamesCache.promise = fetch('/api/plugin-manager/list')
					.then((r) => r.ok ? r.json() : { plugins: [] })
					.then((data) => {
						const names = new Set((data.plugins || []).map((p) => p.name));
						installedPluginNamesCache.names = names;
						return names;
					})
					.catch(() => new Set());
			}
			return installedPluginNamesCache.promise;
		}

		function buildPluginSectionMapSync(installedNames) {
			const cache = (window.__DSH_MODULES__ || {}).loadCache;
			if (!cache || !installedNames || !installedNames.size) return {};
			// Group by npm scope: @scope/pkg → "@scope", unscoped → full name
			const scopeMap = {}; // scopeKey → { displayName, sectionIds: [] }
			for (const [modId, mod] of cache) {
				...
				const slashIdx = modId.indexOf('/');
				const scopeKey = modId.startsWith('@') && slashIdx !== -1
					? modId.substring(0, slashIdx)
					: modId;
				if (!scopeMap[scopeKey]) {
					const displayName = scopeKey.startsWith('@')
						? scopeKey
						: scopeKey.replace(/^(dsh-)?/, '').replace(/-/g, ' ');
					scopeMap[scopeKey] = { displayName, sectionIds: [] };
				}
				scopeMap[scopeKey].sectionIds.push(...sectionIds);
			}
			const map = {};
			for (const { displayName, sectionIds } of Object.values(scopeMap)) {
				map[displayName] = sectionIds;
			}
			return map;
		}
		...
		function HubPanel({ rows, ctx, activeId, onSelect, onClose }) {
			...
			const [pluginSectionMap, setPluginSectionMap] = react.useState({});
			react.useEffect(() => {
				getInstalledPluginNames().then((names) => {
					setPluginSectionMap(buildPluginSectionMapSync(names));
				});
			}, []);
