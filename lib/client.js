...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function buildPluginSectionMap() {
			...
			if (ms && ms.graphRows) {
				for (const [k, v] of ms.graphRows) {
					if (!k.startsWith('@deepseek-ai/')) {
						console.log("[dsh-settings-hub] graphRow:", k, JSON.stringify({url: v.url, external: v.external, inject: v.inject, immediately: v.immediately}));
					}
				}
			}
