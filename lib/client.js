 ...
 *   - every other section collapses under one "插件管理" group
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ORDER_KEY = "dsh-settings-hub-plugin-order";
		function loadPluginOrder() {
			try { return JSON.parse(localStorage.getItem(ORDER_KEY)) ?? []; } catch { return []; }
		}
		function savePluginOrder(ids) {
			try { localStorage.setItem(ORDER_KEY, JSON.stringify(ids)); } catch { /* quota / private */ }
		}
		 ...
		 */
		function PluginNavRow({ row, active, onSelect, onPointerDown, onPointerUp, isDragging, isDragOver }) {
			return jsxRuntime.jsx("button", {
				...
				style: {
					...
					boxShadow: isDragOver ? "inset 0 -2px 0 0 var(--dsw-alias-label-primary)" : "none",
		...
		function HubPanel({ rows, ctx, activeId, onSelect, onClose }) {
			...
			// Custom order (persisted in localStorage); unrecorded IDs appended
			// at the end in their original slot order.
			const [pluginOrder, setPluginOrder] = react.useState(loadPluginOrder);
			const sortedPluginRows = react.useMemo(() => {
				const orderMap = new Map(pluginOrder.map((id, i) => [id, i]));
				return [...pluginRows].sort((a, b) => {
					const ia = orderMap.has(a.id) ? orderMap.get(a.id) : Infinity;
					const ib = orderMap.has(b.id) ? orderMap.get(b.id) : Infinity;
					return ia !== ib ? ia - ib : a.order - b.order;
				});
			}, [pluginRows, pluginOrder]);
			...
			const dragRef = react.useRef({
				...
				draggingId: null, dragOverId: null, sortedRows: [],
			});
			dragRef.current.sortedRows = sortedPluginRows;
			...
			const handlePointerUp = react.useCallback(() => {
				...
				if (ref.draggingId && ref.dragOverId && ref.draggingId !== ref.dragOverId) {
					const ids = ref.sortedRows.map((r) => r.id);
					const si = ids.indexOf(ref.draggingId);
					const ti = ids.indexOf(ref.dragOverId);
					if (si !== -1 && ti !== -1) {
						ids.splice(si, 1);
						ids.splice(ti, 0, ref.draggingId);
						savePluginOrder(ids);
						setPluginOrder(ids);
					}
				}
			...
			react.useEffect(() => {
				const onMove = (e) => {
					...
					// While dragging, midpoint detection: indicator only
					// triggers after the cursor crosses the vertical midpoint
					// of the target row, preventing flicker at boundaries.
					const allRows = [...document.querySelectorAll("[data-plugin-id]")];
					let targetIdx = 0;
					...
					const overId = allRows[targetIdx]?.dataset.pluginId ?? null;
			...
			const [groupOpen, setGroupOpen] = react.useState(sortedPluginRows.some((r) => r.id === active));
									...
									jsxRuntime.jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 4 }, children: [
										...
										pluginRows.length > 0 && jsxRuntime.jsxs("div", { children: [
											...
											groupOpen && jsxRuntime.jsx("div", { children:
												sortedPluginRows.map((row) => jsxRuntime.jsx(PluginNavRow, {
													row,
