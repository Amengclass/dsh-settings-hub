...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function buildPluginSectionMap() {
			...
			if (!cache) return {};
			// Use the authoritative installed-plugins roster — only user-installed
			// plugins appear here, not core/bundled modules.
			const dshPlugins = window.dsh && typeof window.dsh.plugins === 'object' && typeof window.dsh.plugins.list === 'function'
				? window.dsh.plugins.list()
				: null;
			console.log("[dsh-settings-hub] dsh.plugins.list():", dshPlugins);
			console.log("[dsh-settings-hub] loadCache keys:", [...cache.keys()]);
			const ms = window.__DSH_MODULES__;
			console.log("[dsh-settings-hub] __DSH_MODULES__ keys:", ms ? Object.keys(ms) : "null");
			console.log("[dsh-settings-hub] statics:", ms && ms.statics ? (ms.statics instanceof Map ? [...ms.statics.keys()] : typeof ms.statics) : "null");
			console.log("[dsh-settings-hub] graphRows (full):", ms && ms.graphRows ? [...ms.graphRows.entries()].map(([k,v]) => [k, {url:v.url, external:v.external, inject:v.inject}]) : "null");
			if (!dshPlugins || !dshPlugins.length) return {};
			const installedNames = new Set(dshPlugins.filter((p) => p.hasClient).map((p) => p.name));
			console.log("[dsh-settings-hub] installed names:", [...installedNames]);
			const map = {};
			for (const pluginName of installedNames) {
				const mod = cache.get(pluginName);
				...
				const displayName = pluginName.replace(/^(dsh-)?/, '').replace(/-/g, ' ');
				const sections = [];
				const idMatches = [...src.matchAll(/id:\s*["']([^"']+)["']/g)];
				for (const m of idMatches) {
					if (!NATIVE_IDS.has(m[1])) sections.push(m[1]);
				}
				if (sections.length > 0) map[displayName] = sections;
			}
			return map;
		}
		...
		function HubPanel({ rows, ctx, activeId, onSelect, onClose }) {
			...
			const pluginSectionMap = react.useMemo(() => buildPluginSectionMap(), []);
