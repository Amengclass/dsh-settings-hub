...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Scan loadCache for non-core modules that register settings.section
		 * entries. Each module ID becomes its own group (root plugins register
		 * directly; sub-dependencies surface under their own name).
		 * No API dependency — works even when plugin-manager API is absent.
		 */
		function buildPluginSectionMap() {
			...
			for (const [modId, mod] of cache) {
				...
				const displayName = modId.startsWith('@')
					? modId
					: modId.replace(/^(dsh-)?/, '').replace(/-/g, ' ');
